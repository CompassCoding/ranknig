exports.run = async (Discord, client, message, [Username, ...Reason], args) => {
 ...
 if(!client.premiumusers.has(message.guild.ownerID)) {
        ...
        if (Username) {
              ...
              if (Reason) {
                  rbx.getIdFromUsername(Username.toLowerCase()).then(id => {
                      if (userid && Reason) {
                        ...
                        RichEmbed(Moderator, 0XFF5151, 'Invalid Arguments Provided.\n\nUsage: `demoted <ROBLOX user/id> <reason>`');
                      };
                }).catch(e => {msg.reply(e)});
              } else {
                  RichEmbed(Moderator, 0XFF5151, 'Please provide the reason of why you want to demoted the ROBLOX user.\n\nUsage: `demoted <ROBLOX user/id> <reason>`');
              };
          } else {
              RichEmbed(Moderator, 0XFF5151, 'Invalid Arguments Provided.\n\nUsage: `demoted <ROBLOX user/id> <reason>`');
          };
          ...
          function MessageManage(Moderator, Color, Description) {
              var embed = new Discord.RichEmbed()
