        ...
        if (Username) {
            const userid = await rbx.getIdFromUsername(Username);
              if (Reason) {
                  rbx.getIdFromUsername(Username.toLowerCase()).then(id => {
                      if (userid && Reason) {
                        ...
                        if (Rank != null && Rank + 1 < 255 && Rank != 0) {
                          rbx.exile({group: group, target: id}).then(async () => {
                              const RankAfter = await rbx.getRankInGroup({group: group, userId: id});
                              rbx.follow({userId: id}).catch(e=> {msg.reply(e)}); 
                              rbx.message({ recipient: id, subject: 'Ranking', body: 'You have been Exiled in ' + group.name + ' by ' + msg.member.displayName + ' to ' + Rank + '.\nIf you believe this is an error, feel free to contact an SHR about this.' }).then(() => {
                              rbx.getRankNameInGroup(group, id).then(rankBefore1 => {
                                  var rankBefore = rankBefore1;
                              });
                                  RichEmbed(Moderator, 0X42F47A, 'User has been successfully Exiled!');
                                  MessageManage(Moderator, 0XFFFC56, 'User has been Exiled through Discord.\n\nModerator: `' + Moderator.tag + '`\nROBLOX User: `' + Username + '`\nRanked From: `' + Rank + '`\nRanked To: `' + RankAfter + '`\nReason: `' + Reason + '`');
                              }).catch(e => {
                                  RichEmbed(Moderator, 0XFFFC56, 'User has been successfully Exiled, but unable to be messaged.');
                                  MessageManage(Moderator, 0XFFFC56, 'User has been Exiled through Discord.\n\nModerator: `' + Moderator.tag + '`\nROBLOX User: `' + Username + '`\nRanked From: `' + Rank + '`\nRanked To: `' + RankAfter + '`\nReason: `' + Reason + '`');
                              });
                          }).catch(e => {
                              RichEmbed(Moderator, 0XFF5151, 'Please make sure the user is able to exile the mentioned users, that the user is in the group!');
                          });
                        } else {
                  ...
                  RichEmbed(Moderator, 0XFF5151, 'Please provide the reason of why you want to exile the ROBLOX user.\n\nUsage: `exile <ROBLOX user/id> <reason>`');
              };
          } else {
              RichEmbed(Moderator, 0XFF5151, 'Invalid Arguments Provided.\n\nUsage: `exile <ROBLOX user/id> <reason>`');
          };
